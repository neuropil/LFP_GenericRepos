function dat = calc_chn_pwr_JAT(data,opt,spectype)
...
% rem_nan: [logical] = remove NaNs from vector
%
%Usage example: 
...
%%%% JAT Modify 7/21/2015 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isfield(opt,'rem_nan')
    if opt.rem_nan == 1;
        data = data(~isnan(data));
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~isfield(opt,'seg_len'); 
	opt.seg_len = 2; 
end %sec

if ~isfield(opt,'pad'); 
	opt.nfft = 2^11; % was 2^9
else 
	opt.nfft = opt.pad; 
end %sec

%%%% JAT Modify 7/18/2015 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sizeData = size(data);
...
if Nchns == 1
    [ts_out,xStart,xEnd] = create_ts_mtaper(data',eplen,0,fs );
else
...
%Calculate complex spectral estimations using multitapers
	...
	error('Must specify spectral estimation type')
end
