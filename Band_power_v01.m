...
function [specdat] = Band_power_v01()
%% isolating  data

load('LFP_06052014.mat')
...
for i=1:length(LFPdataST.depthVals)
    ...
    opt.rem_nan = 1; % Parameter that will remove NANs
%     var1 = dat1(~isnan(dat1));
%     var2 = dat2(~isnan(dat2));
%     var3 = dat3(~isnan(dat3));
    specdat.lfp1.c = calc_chn_pwr_JAT(dat1,opt,'mtaper');
    specdat.lfp2.c = calc_chn_pwr_JAT(dat2,opt,'mtaper');
    specdat.lfp3.c = calc_chn_pwr_JAT(dat3,opt,'mtaper');
    
    for n= 1:3 %loop for CLFP1,2,3 data
        if n==1
            transformed_signal=specdat.lfp1.c.mean_spec;
        end
        if n==2
            transformed_signal=specdat.lfp2.c.mean_spec;
        end
        if n==3
            transformed_signal=specdat.lfp3.c.mean_spec;
        end
        frequency_scale=specdat.lfp1.c.freq;
